...
public class Editor extends JFrame {	
	...
	public void drawSketch(Graphics g) {
		// TODO: YOUR CODE HERE
		for (Integer id : sketch.getShapeTree().navigableKeySet()) {
				sketch.getShapeTree().get(id).draw(g);
		}
		// draw curr shape on individual editor, but don't add to master sketch until released
		if (curr!=null) {
			curr.draw(g);
		}
	}
	...
	private void handlePress(Point p) {
		...
		if (mode == Mode.DRAW) {
			...
			// look at top shapes first, thus use descendingKeySet
			for (Integer id : sketch.getShapeTree().descendingKeySet()) {
				if (sketch.getShapeTree().get(id).contains((int)p.getX(),(int)p.getY())) {
					moveFrom = p;
					movingId = id;
					break;
				}
